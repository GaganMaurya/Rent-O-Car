...
import com.example.adminrentocar.databinding.ActivityFirstPagerv1Binding
...
class firstPagerv1 : AppCompatActivity() {
    lateinit var binding : ActivityFirstPagerv1Binding
    private var id = 123
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityFirstPagerv1Binding.inflate(layoutInflater)
    ...
    private fun uploaddata() {
        ...
        firbaseStorage.getReference("Car-Details").child(binding.carname.text.toString())
                ...
                task.metadata?.reference?.downloadUrl
                    ?.addOnSuccessListener {
                        val map = hashMapOf(
                            ...
                            "range" to binding.range.text.toString(),
                            "power" to binding.power.text.toString(),
                            "miles" to binding.miles.text.toString(),
                            "speed" to binding.speed.text.toString(),
                        )
                        firebaseref =
                            FirebaseDatabase.getInstance().getReference("Car-Details")
                        firebaseref.child(binding.carname.text.toString()).setValue(map)
                            .addOnSuccessListener {
                                ...
                                binding.range.text.clear()
                                binding.power.text.clear()
                                binding.miles.text.clear()
                                binding.speed.text.clear()
                                pro?.dismiss()
